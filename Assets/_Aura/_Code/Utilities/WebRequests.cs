...
{
	//contained WebRequestsMonobehaviour instance
	private static WebRequestsMonobehaviour webRequestsMonoBehaviour;
	private static void Init()
	{
		if(webRequestsMonoBehaviour == null)
		{
			//set it up in the scene so we can run
			//normal game loop call backs
			GameObject webRequestsObject = new GameObject();
			webRequestsMonoBehaviour = webRequestsObject.AddComponent<WebRequestsMonobehaviour>();
		}
	}

    #region public HTTP API
    ...
    {
        Init();
        webRequestsMonoBehaviour.StartCoroutine(GetCoruoutineDelete(url, onError, onSuccess));
    }

    #endregion

    #region Get Coroutines
    ...
    #region Delete Coroutine
    private static IEnumerator GetCoruoutineDelete(string url, Action<string> onError, Action<string> onSuccess)
    {
    ...
    #region Contained Utility Monobehaviour
